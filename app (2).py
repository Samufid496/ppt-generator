...
def fit_image_in_box(img_w_px, img_h_px, box_w_emu, box_h_emu):
    """
    Scale image to fit inside the box while keeping aspect ratio.
    Returns (left_offset, top_offset, final_width, final_height) in EMU
    — offsets are relative to the box top-left, for centering.
    """
    img_ratio = img_w_px / img_h_px
    box_ratio  = box_w_emu / box_h_emu

    if img_ratio > box_ratio:
        # Image is wider → fit by width
        final_w = box_w_emu
        final_h = int(box_w_emu / img_ratio)
    else:
        # Image is taller → fit by height
        final_h = box_h_emu
        final_w = int(box_h_emu * img_ratio)

    # Center inside the box
    offset_x = (box_w_emu - final_w) // 2
    offset_y = (box_h_emu - final_h) // 2

    return offset_x, offset_y, final_w, final_h

def get_placeholder_pos(slide):
    """Find imgProduct shape, grab its position/size, remove it."""
...
def update_slide(slide, row, image_bytes=None):
    ...
    box_left, box_top, box_w, box_h = get_placeholder_pos(slide)
    ...
    if image_bytes:
        ...
        # Calculate fitted position (no stretch)
        off_x, off_y, final_w, final_h = fit_image_in_box(img_w, img_h, box_w, box_h)
...
st.markdown('<div class="tip">💡 Name each image exactly like the Material code — e.g. <code>MI-PIN200-06CY.jpg</code><br>Images fit inside the container <b>without stretching</b>, centered automatically.</div>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
