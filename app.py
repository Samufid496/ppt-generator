...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="PPT Generator | Pan Gulf Marketing",
    page_icon="📊",
    layout="centered"
)
...
<style>
    ...
    h1 { color: #1E2761; }
    .banner {
        background: linear-gradient(135deg, #00ADEE, #1E2761);
        border-radius: 12px;
        padding: 28px 32px;
        margin-bottom: 28px;
        color: white;
...
def update_slide(slide, row, image_bytes=None):
    for shape in slide.shapes:
        ...
        if shape.has_table:
            ...
            font_sizes = [None, None, 11, None, None, None]
            for i, cell in enumerate(dr.cells):
                set_cell_text(cell, vals[i], font_sizes[i])

    if image_bytes:
        img_width  = Inches(3.5)
        img_height = Inches(2.8)
        left = (Inches(13.33) - img_width) / 2
        top  = Inches(1.4)
        slide.shapes.add_picture(image_bytes, left, top, img_width, img_height)
...
st.markdown('<div class="tip">💡 This is your <b>Discounter_working.pptx</b> master slide.</div>', unsafe_allow_html=True)
...
st.markdown('<div class="tip">💡 Name each image exactly like the Material code. Example: <code>MI-PIN200-06CY.jpg</code></div>', unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
output_name = st.text_input("Output filename", value="Output.pptx", label_visibility="visible")
...
generate_btn = st.button(
    ...
    type="primary",
    disabled=not (template_file and df_preview is not None),
    ...
    if missing:
        ...
        for result in gen:
            if isinstance(result[0], int):
                ...
                progress_bar.progress(pct, text=f"Processing slide {done} of {total}…")
            else:
...
st.markdown("---")
st.markdown(
    "<p style='text-align:center; color:#999; font-size:0.8rem;'>Pan Gulf Marketing © 2025 · Built with Streamlit</p>",
    unsafe_allow_html=True
)
